...
DATA_DIR="SPModel/ReproducePNAS4ItemsV2"
MATLAB_SOURCE_PATH='SpikeNet'
MATLAB_SOURCE_PATH_2='myfunctions'
MATLAB_PRE_PROCESS_FUNC="main_Gamma_SP"
MATLAB_MIDDLE_PROCESS_FUNC="ModifyCurrentNetworkSet"
...
if [ "${indone}" == "1" ]; then
	...
	echo Modifying current network setting;
	matlab -singleCompThread -nodisplay  -r "cd('${MATLAB_SOURCE_PATH}'), addpath(genpath(cd)), cd('${PBS_O_WORKDIR}') , \
						   ...
						   cd('${DATA_DIR}'), ${MATLAB_MIDDLE_PROCESS_FUNC}(${PBS_ARRAYID}), exit" # Create ${INPUT_APPENDIX} file
	cd ${DATA_DIR};        
